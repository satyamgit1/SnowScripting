...
export default function Careers() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name || !formData.email || !formData.position || !formData.experience || !formData.resume) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSubmitStatus({ success: false, message: result.message || 'Submission failed. Please try again.' });
      }
    } catch (error) {
      setSubmitStatus({ success: false, message: 'Network error. Please try again.' });
    } finally {
