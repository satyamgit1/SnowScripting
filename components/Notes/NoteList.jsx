...
const GlowingCard = ({ children }) => (
  <motion.div
    whileHover={{ y: -5, boxShadow: '0 10px 25px -5px rgba(74, 222, 128, 0.2)' }}
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 p-6">
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          {/* Script Type Filter */}
          <div>
            <label className="block text-sm font-medium text-gray-400 mb-2">Script Type</label>
            <div className="relative">
              <select
                value={filters.scriptType}
                onChange={(e) => setFilters({...filters, scriptType: e.target.value})}
              ...
              >
                ...
                <option value="business_rule" className="bg-gray-800">Business Rule</option>
                <option value="script_include" className="bg-gray-800">Script Include</option>
                <option value="client_script" className="bg-gray-800">Client Script</option>
                <option value="ui_action" className="bg-gray-800">UI Action</option>
                <option value="scheduled_job" className="bg-gray-800">Scheduled Job</option>
                <option value="email_template" className="bg-gray-800">Email Template</option>
                <option value="ui_script" className="bg-gray-800">UI Script</option>
                <option value="widget" className="bg-gray-800">Widget</option>
                <option value="fix_script" className="bg-gray-800">Fix Script</option>
                <option value="background_script" className="bg-gray-800">Background Script</option>
              </select>
          ...
          <div>
            ...
            <div className="relative">
              ...
              >
                ...
                {uniqueProjects.map(project => (
                  <option key={project} value={project} className="bg-gray-800">{project}</option>
                ))}
              </select>
              <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-emerald-500">
                <svg className="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
            ...
            {totalPages > 1 && (
              ...
              >
                <motion.button
                  ...
                  onClick={() => paginate(Math.max(1, currentPage - 1))}
                  disabled={currentPage === 1}
                  className={`p-2 rounded-xl ${currentPage === 1 ? 'text-gray-600 cursor-not-allowed' : 'text-emerald-400 hover:bg-gray-700'}`}
                >
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                  </svg>
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((number) => (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.9 }}
                    onClick={() => paginate(number)}
                    className={`w-10 h-10 flex items-center justify-center rounded-xl ${currentPage === number ? 'bg-gradient-to-r from-emerald-500 to-green-500 text-white shadow-md' : 'text-emerald-400 hover:bg-gray-700'}`}
                  >
                    {number}
                  </motion.button>
                ))}
                ...
                <motion.button
                  ...
                  onClick={() => paginate(Math.min(totalPages, currentPage + 1))}
                  disabled={currentPage === totalPages}
                  className={`p-2 rounded-xl ${currentPage === totalPages ? 'text-gray-600 cursor-not-allowed' : 'text-emerald-400 hover:bg-gray-700'}`}
                >
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                  </svg>
