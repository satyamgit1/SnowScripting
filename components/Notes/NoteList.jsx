...
const AnimatedBackground = () => (
  <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
    ...
    {[...Array(5)].map((_, i) => (
      <motion.div
        key={`orb-${i}`}
        className="absolute rounded-full bg-gradient-to-br from-emerald-400/10 to-green-300/10 filter blur-xl"
        style={{
...
export default function NoteList({ userId }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-900 to-gray-800 p-2 relative overflow-hidden">
      {/* Animated background */}
      <AnimatedBackground />
      ...
      <motion.div
        ...
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.2 }}
        className="bg-gray-800/80 backdrop-blur-md p-6 rounded-3xl shadow-xl border border-gray-700/50 mb-12 relative z-10"
      >
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-200 flex items-center mb-4 md:mb-0">
            <svg
            ...
            <label className="flex items-center cursor-pointer">
              <div className="relative">
                <input
                  ...
                  onChange={() => setUseAdvancedSearch(!useAdvancedSearch)}
                />
                <div className={`block w-14 h-7 rounded-full transition-colors duration-200 ease-in-out ${
                  useAdvancedSearch ? 'bg-emerald-500' : 'bg-gray-600'
                }`}>
                  <div className={`absolute left-1 top-1 bg-white w-5 h-5 rounded-full transition-transform duration-200 ease-in-out ${
                    useAdvancedSearch ? 'transform translate-x-7' : ''
                  }`}></div>
                </div>
              </div>
              <span className="ml-3 text-gray-300 font-medium">Use Advanced Search</span>
            </label>
        ...
        {/* Advanced Filters - shown when checkbox is checked */}
        <AnimatePresence>
          {useAdvancedSearch && (
            <motion.div
      ...
      <AnimatePresence>
        {filteredNotes.length === 0 ? (
          <motion.div
            ...
            animate={{ opacity: 1, scale: 1 }}
            exit={{ opacity: 0 }}
            className="bg-gray-800/80 backdrop-blur-md p-12 rounded-3xl border-2 border-dashed border-emerald-700 text-center relative z-10"
          >
            <svg
              className="mx-auto h-16 w-16 text-emerald-400"
              fill="none"
          ...
          <>
            ...
            {totalPages > 1 && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                transition={{ delay: 0.2 }}
                className="flex items-center justify-center space-x-2 mb-12 relative z-10"
              >
                <motion.button
                  whileHover={{ scale: currentPage !== 1 ? 1.1 : 1 }}
                  whileTap={{ scale: currentPage !== 1 ? 0.9 : 1 }}
