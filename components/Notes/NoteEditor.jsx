...
export default function NoteEditor({ initialData, onSave, projectList = [] }) {
  ...
  // Fixed: Get unique projects properly from projectList
  const allProjects = projectList
    .map((item) => item.project)
    .filter((project) => project && project.trim() !== "");
  
  // Use a Set to get unique values (case insensitive)
  const projectSet = new Set();
  const uniqueProjects = [];
  
  for (const project of allProjects) {
    const lowerCaseProject = project.toLowerCase();
    if (!projectSet.has(lowerCaseProject)) {
      projectSet.add(lowerCaseProject);
...
Sent from ServiceNow Script Manager`;
  ...
  // Fixed: Update the isDuplicateProjectName function
  function isDuplicateProjectName(name) {
    if (!name?.trim()) return false;
    const nameLower = name.trim().toLowerCase();
    return uniqueProjects.some(
      (project) => project.toLowerCase() === nameLower
    );
  }
  ...
  }, [formData.project, uniqueProjects]);
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="space-y-6 bg-gray-900 p-6 rounded-2xl shadow-lg relative overflow-hidden border border-gray-800">
        ...
        >
          ...
          <motion.div
            className="bg-gray-800 p-4 rounded-xl border border-gray-700 shadow-sm hover:shadow-md transition-shadow project-dropdown-container"
            whileHover={{ y: -2 }}
          >
            <label
              htmlFor="project"
            ...
            {!showNewProjectInput && (
              <div className="relative">
                <motion.div
                  className="block w-full pl-10 pr-10 py-2 border border-gray-700 rounded-lg bg-gray-700 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 appearance-none transition-all shadow-sm text-white cursor-pointer min-h-[40px] flex items-center"
                ...
                <AnimatePresence>
                  {showProjectDropdown && (
                    <motion.div
                      ...
                      transition={{ duration: 0.2 }}
                    >
                      {/* FIXED: Show existing projects if available */}
                      {uniqueProjects.length > 0 ? (
                        <>
                          ...
                          {uniqueProjects.map((project) => (
                            <motion.div
                              key={project}
                              className="px-4 py-2 hover:bg-gray-700 cursor-pointer text-gray-200 flex items-center"
                      ...
                      <motion.div
                        className="px-4 py-2 hover:bg-gray-700 cursor-pointer text-emerald-400 flex items-center border-t border-gray-700"
            ...
            {showNewProjectInput && (
              <div className="flex space-x-2">
                <motion.input
                  ...
                  className={`flex-1 pl-10 pr-3 py-2 border ${
                    projectError ? "border-red-500" : "border-gray-700"
                  } rounded-lg bg-gray-700 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all shadow-sm text-white placeholder-gray-400`}
