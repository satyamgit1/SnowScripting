...
//   );
// };
// export default function NoteEditor({ initialData, onSave, onDataChange, hideActionButtons = false, projectList = [] }) {
//   const { user } = useAuth();
//   const [formData, setFormData] = useState(
//     initialData || {
//       title: "",
...
//       setProjectError(null);
//     }
//   }, [formData.project, formData.isPersonalNote, projectList, initialData]);
//   const FloatingParticles = () => {
//     const particles = Array.from({ length: 15 }, (_, i) => {
//       const size = Math.random() * 5 + 2;
//       return {
...
//     <div className="container mx-auto p-4">
//       <ServiceNowAIAssistant />
//       <div className="space-y-6 bg-gray-900 p-6 rounded-2xl shadow-lg relative overflow-visible border border-gray-800">
...
//         </div>
//         <motion.div
//           className="flex justify-between items-center pb-6 border-b border-emerald-800 relative z-10"
//           initial={{ opacity: 0, y: -20 }}
//           animate={{ opacity: 1, y: 0 }}
//           transition={{ duration: 0.5 }}
//         >
...
//           </div>
//           {formData.content && (
//             <motion.div className="relative z-50" whileHover={{ scale: 1.05 }}>
//               {/* <motion.button
//                 onClick={() => setShowDownloadMenu(!showDownloadMenu)}
//                 className="flex items-center px-4 py-2 bg-gradient-to-r from-emerald-500 to-green-600 text-white rounded-lg hover:from-emerald-600 hover:to-green-700 transition-all shadow-md hover:shadow-lg group relative overflow-hidden"
//                 whileTap={{ scale: 0.95 }}
...
//                 {showDownloadMenu && (
//                   <motion.div
//                     className="absolute right-0 mt-2 w-56 bg-gray-800 rounded-lg shadow-xl z-[9999] border border-emerald-900 overflow-hidden"
//                     initial={{ opacity: 0, y: -10 }}
//                     animate={{ opacity: 1, y: 0 }}
//                     exit={{ opacity: 0, y: -10 }}
//                     transition={{ duration: 0.2 }}
...
//                     onChange={handleScriptTypeChange}
//                     className="block w-full pl-10 pr-10 py-2 border border-gray-700 rounded-lg bg-gray-700 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 appearance-none transition-all shadow-sm text-white"
//                     required={!formData.isPersonalNote}
//                     whileFocus={{ scale: 1.01 }}
//                   >
//                     {scriptTypes.map((type) => (
//                       <option key={type} value={type} className="bg-gray-800">
...
//             )}
//           </div>
//           {!formData.isPersonalNote && scriptSubTypes[formData.scriptType]?.length > 0 && (
//             <motion.div
//               className="bg-gray-800 p-4 rounded-xl border border-gray-700 shadow-sm hover:shadow-md transition-shadow"
//               initial={{ opacity: 0, height: 0 }}
//               animate={{ opacity: 1, height: "auto" }}
...
//                 type="checkbox"
//                 id="personalNote"
//                 checked={formData.isPersonalNote}
//                 onChange={(e) => setFormData({ ...formData, isPersonalNote: e.target.checked })}
//                 className="h-4 w-4 text-emerald-600 focus:ring-emerald-500 border-gray-300 rounded"
//               />
//               <label htmlFor="personalNote" className="ml-2 block text-sm text-gray-300">
//                 Personal Note (no project required)
//               </label>
//             </div>
//           </motion.div>
//           {!formData.isPersonalNote && (
...
//           {/* Only show action buttons if hideActionButtons is false */}
//           {!hideActionButtons && (
//             <>
//               <motion.div
//                 className="flex justify-end space-x-4 pt-4 border-t border-emerald-800"
...
//   // Handle personal note checkbox change
//   useEffect(() => {
//     if (formData.isPersonalNote) {
//       // Set script type to personal_note
//       setFormData(prev => ({
//         ...prev,
//         scriptType: "personal_note",
//         project: "Personal Notes"
//       }));
//     } else {
//       // Reset to default values if not personal note
//       setFormData(prev => ({
//         ...prev,
//         scriptType: "business_rule",
//         scriptSubType: "before",
...
export default function NoteEditor({ initialData, onSave, onDataChange, hideActionButtons = false, projectList = [] }) {
  const { user } = useAuth();
  const [formData, setFormData] = useState(() => {
    // Initialize with initialData if provided, otherwise use defaults
    return initialData || {
      title: "",
      content: "",
      scriptType: "business_rule",
